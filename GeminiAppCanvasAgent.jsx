...
import { formatReactValue, generateAgentPrompt } from './gemini_logic.js';
...
window.cli_internals = {
  ...
  runAgent: async function(args, context) {
      ...
      try {
        ...
        const prompt = generateAgentPrompt(goal, memories, toolList);
  ...
  inspect: function(path) {
      ...
      try {
          return formatReactValue(path.split('.').reduce((o, k) => o[k], window), 0, 10);
      } catch (e) {
  ...
  executeCode: function(code) {
      try {
          ...
          return `=> ${formatReactValue(result, 0, 10)}`;
      } catch (e) {
          return `Error: ${e.message}`;
      }
  },
  ...
  useEffect(() => {
    ...
    consoleMethods.forEach(method => {
      console[method] = (...args) => {
        ...
        const output = args.map(arg => formatReactValue(arg, 0, 5)).join(' ');
