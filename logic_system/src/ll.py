...
def tensor_right(
    left_proof: ProofTree, right_proof: ProofTree, formula: Tensor
) -> ProofTree:
    """Γ ⊢ Δ, A   and   Γ' ⊢ Δ', B
    --------------------------------
          Γ, Γ' ⊢ Δ, Δ', A ⊗ B
    ...
    B = formula.right
    if (
        A not in left_proof.conclusion.succedent
        or B not in right_proof.conclusion.succedent
    ):
...
def par_right(proof: ProofTree, formula: Par) -> ProofTree:
    ...
    """
    if (
        formula.left not in proof.conclusion.succedent
        or formula.right not in proof.conclusion.succedent
    ):
        raise ValueError("Premises do not support the conclusion for ⅋-R")
    new_succedent = (
...
def par_left(left_proof: ProofTree, right_proof: ProofTree, formula: Par) -> ProofTree:
    ...
    B = formula.right
    if (
        A not in left_proof.conclusion.antecedent
        or B not in right_proof.conclusion.antecedent
    ):
        raise ValueError("Premises do not support the conclusion for ⅋-L")
...
def cut(left_proof: ProofTree, right_proof: ProofTree) -> ProofTree:
    ...
    """
    cut_formulas = (
        left_proof.conclusion.succedent.keys()
        & right_proof.conclusion.antecedent.keys()
    )
    if len(cut_formulas) != 1:
        raise ValueError(
            f"Cut rule requires exactly one common formula, but found {len(cut_formulas)}"
        )
    ...
    succedent = (
        left_proof.conclusion.succedent + right_proof.conclusion.succedent
    ) - Counter([cut_formula])
    conclusion = Sequent(antecedent, succedent)
