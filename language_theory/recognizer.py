...
def parse_grammar(filepath):
    ...
    with open(filepath, 'r') as f:
        for line in f:
            ...
            non_terminal, rules_str = line.split('->', 1)
            non_terminal = non_terminal.strip()

            # Split rule into symbols by treating each character as a symbol.
            # This works for the single-character convention we're using.
            parsed_rules = [list(r.strip()) for r in rules_str.split('|')]
            grammar[non_terminal].extend(parsed_rules)
...
def main():
    ...
    try:
        ...
        # Heuristic: Check if the grammar conforms to right-linear or left-linear rules.
        is_right_reg = all(len(r) <= 2 and (len(r) < 2 or r[1].isupper()) for rules in grammar.values() for r in rules)
        is_left_reg = all(len(r) <= 2 and (len(r) < 2 or r[0].isupper()) for rules in grammar.values() for r in rules)

        # Mutually exclusive regular grammars are handled first.
        if is_right_reg and not is_left_reg:
        ...
        # Otherwise, fall back to the more powerful Earley parser.
        else:
            print("Heuristic determined grammar to be CONTEXT-FREE or ambiguous regular. Using Earley parser.")
