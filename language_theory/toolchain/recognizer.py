...
def recognize_earley(grammar_productions, start_symbol, input_tokens):
    ...
    for rule_rhs in grammar_productions.get(start_symbol, []):
        chart[0].append(EarleyItem((start_symbol, rule_rhs), 0, 0))
    ...
    for i in range(len(input_tokens) + 1):
        ...
        while item_idx < len(chart[i]):
            ...
            item_idx += 1
            if item.dot_pos < len(item.rule[1]): # Incomplete items
                next_symbol = item.rule[1][item.dot_pos]
                if next_symbol.isupper(): # Predictor
                ...
                elif i < len(input_tokens) and next_symbol == input_tokens[i]: # Scanner
...
def count_parses(item):
    if not item.back_pointers: return 1
    if isinstance(item.back_pointers[0], str): return 1
...
def main():
    ...
    try:
        ...
        is_right_reg = all(len(rhs) <= 2 and (len(rhs) < 2 or rhs[1].isupper()) for _, rhs in grammar.productions) and not is_csg and not is_contracting and all(len(lhs) == 1 for lhs, _ in grammar.productions)
        is_left_reg = all(len(rhs) <= 2 and (len(rhs) < 2 or rhs[0].isupper()) for _, rhs in grammar.productions) and not is_csg and not is_contracting and all(len(lhs) == 1 for lhs, _ in grammar.productions)
        ...
        if is_contracting:
            ...
            print("Heuristic: RIGHT-LINEAR REGULAR. (Note: Only simple recognizer implemented)")
            print(f"\nFAILURE: Recognition for this grammar type is not fully supported in this version.")
        elif is_left_reg and not is_right_reg:
            print("Heuristic: LEFT-LINEAR REGULAR. (Note: Only simple recognizer implemented)")
            print(f"\nFAILURE: Recognition for this grammar type is not fully supported in this version.")
        else:
