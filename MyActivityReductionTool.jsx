...
export default function App() {
    ...
    const [interactions, setInteractions] = useState([]); // Holds ALL parsed interactions
    ...
    const [isProcessingHeavyTask, setIsProcessingHeavyTask] = useState(false); // Declared
    ...
    const downloadReport = (content, filename, mimeType = 'text/plain') => { /* ... (same as before) ... */ };
    const callGeminiAPI = async (promptForLLM, signal) => { /* ... (same as before) ... */ };
    const parseBatchedAnalysisResponse = (batchedResponse, batchOriginalIndices, batchTimestamps) => { /* ... (same as before) ... */ };
    const reProcessInteractionForFiltersAndEncodings = (interaction, transformationApplied) => { /* ... (same as before) ... */ };
    ...
    const handleGenerateSemanticEncodings = () => { /* ... (same as before) ... */ };

    // Optimized Anti-Zipfian Application
    const handleApplyAntiZipfianFilter = async () => {
        ...
        // const rareWordsSet = new Set(rareWordsFound); // Optimization if .some becomes slow

        for (let k = 0; k < interactions.length; k++) {
            const int = interactions[k];
            // Important: Filter on the *current* text state (original or vowel-removed)
            const interactionText = `${int.query} ${int.response}`.toLowerCase(); 
            const interactionTokens = tokenizeTextForFrequency(interactionText); // Tokenize once
    ...
    const prepareForBulkAnalysis = () => { /* ... (same as before) ... */ };
    const handleCancelAnalysis = () => { /* ... (same as before) ... */ };
    const downloadInteractionsAsJSON = () => { /* ... (same as before) ... */ };
    const downloadInteractionMetadata = () => { /* ... (same as before) ... */ };
    ...
    const displayedInteractions = useMemo(() => {
        ...
        if (isAntiZipfianFilterActive) {
            filtered = filtered.filter(int => matchingInteractionIdsForAntiZipfian.has(int.id));
        }
    ...
    const handleConfirmBulkAnalysis = () => {
        const interactionsForBulk = displayedInteractions.filter(int => int.clientSideFilter.passed);
        startBulkAnalysis(interactionsForBulk);
    };


    if (selectedInteraction) {
        // Interaction Detail View - (This JSX structure is simplified for brevity, full structure from previous turn)
                            ...
                            <div className="mt-4 p-3 border dark:border-gray-700 rounded-md bg-gray-50 dark:bg-gray-700/50">
                                ...
                                <div className="flex items-center space-x-2 mt-1">
                                    ...
                                    <button onClick={handleApplyAntiZipfianFilter} disabled={!wordFrequenciesGlobal || isProcessingHeavyTask} className="bg-pink-500 hover:bg-pink-600 text-white font-semibold py-1 px-3 rounded-md text-sm disabled:opacity-50">
                                        {isProcessingHeavyTask && isApplyingAntiZipfian ? 'Applying...' : 'Apply Filter'}
                                    </button>
