...
def main():
    ...
    decider = ParaconsistentHaltingDecider(
        plan_path=args.plan_path, max_steps=args.max_steps
    )
    ...
    if final_state.value == ParaconsistentTruth.BOTH:
        ...
        print(
            "2. The UDC program reads this state (H=F) and, as per its logic, proceeds to the HALT instruction."
        )
        print(
            "3. Executing HALT adds True to the halting state. The state becomes H = {True, False} -> 'BOTH'."
        )
        print(
            "4. The LFI model successfully contains the paradox without trivializing (exploding). It concludes that the program is in a contradictory, but stable, state."
        )
        print(
