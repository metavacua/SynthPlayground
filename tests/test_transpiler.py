...
"""
        ...
        with open(output_path, "r") as f:
            ...
            # NEW: Check for the correct, refactored output
            self.assertIn(
                "from toolchain.plang_runtime import ParaconsistentVariable",
                transpiled_code,
            )
            self.assertIn(
                'config = ParaconsistentVariable({"StanceA": {"value": 1}, "StanceB": {"value": 2}})',
                transpiled_code,
            )
            self.assertIn('x = config.resolve("StanceA")', transpiled_code)
