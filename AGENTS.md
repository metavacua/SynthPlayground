# Agent Protocol Specification: Agent-Action-Language (AAL)

This document contains the complete and authoritative set of protocols governing agent behavior in this repository. It is generated programmatically from the source files located in the `protocols.aal/` directory.

## The AAL System

The protocols in this repository are defined using the **Agent-Action-Language (AAL)**, a unified format that combines structured data with human-readable documentation.

*   **Source of Truth:** The `.aal` files in `protocols.aal/` are the single source of truth.
*   **Compilation:** This `AGENTS.md` file is generated by the `tooling/aal_compiler.py` script.
*   **DO NOT EDIT THIS FILE DIRECTLY.** All changes must be made to the source `.aal` files.
