...
class TestDependencyGraphGenerator(unittest.TestCase):
    ...
    def setUp(self):
        ...
        os.makedirs(
            os.path.join(self.test_dir, "project_b", "node_modules", "some_dep"),
            exist_ok=True,
        )
        ...
        with open(self.pkg_a_path, "w") as f:
            json.dump(
                {
                    "name": "project-a",
                    "dependencies": {"project-b": "1.0.0", "external-lib": "2.0.0"},
                },
                f,
            )
        ...
        with open(self.pkg_b_path, "w") as f:
            json.dump({"name": "project-b", "devDependencies": {"jest": "29.0.0"}}, f)

        # Create a package.json inside node_modules that should be ignored
        ignored_pkg_path = os.path.join(
            self.test_dir, "project_b", "node_modules", "some_dep", "package.json"
        )
