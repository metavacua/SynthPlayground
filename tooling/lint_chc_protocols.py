...
def lint_chc_protocols():
    ...
    if not os.path.isdir(protocol_dir):
        logging.error(f"CHC protocol directory not found at '{protocol_dir}'. Aborting.")
        return False
    ...
    for root, _, files in os.walk(protocol_dir):
        if "proof.py" in files:
            ...
            module_path = os.path.join(root, "proof.py").replace(os.path.sep, ".").replace(".py", "")
            ...
            try:
                ...
                if not hasattr(protocol_module, 'Protocol'):
                    logging.error(f"Protocol '{module_path}' is missing the 'Protocol' class.")
                    has_errors = True
                    continue
                ...
                missing_methods = [method for method in required_methods if not hasattr(protocol_instance, method)]

                if missing_methods:
                    logging.error(f"Protocol '{module_path}' is missing the following required methods: {', '.join(missing_methods)}")
                    has_errors = True
                else:
                    logging.info(f"Protocol '{module_path}' correctly implements the CHCProtocol interface.")

            except ImportError as e:
                logging.error(f"Failed to import protocol from '{module_path}': {e}")
                has_errors = True
            except Exception as e:
                logging.error(f"An unexpected error occurred while linting protocol '{module_path}': {e}")
                has_errors = True
