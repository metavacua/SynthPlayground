...
def execute_build(target_name, config):
    ...
    try:
        ...
        print(f"  - Error:    Build process for '{target_name}' failed with exit code {e.returncode}.")
        print("  - STDERR:")
...
def main():
    ...
    """
    parser = argparse.ArgumentParser(description="Unified build script for the project.")
    parser.add_argument(
        "--target",
        "-t",
        required=True,
    ...
    if args.list_targets:
        ...
        for name, details in config['targets'].items():
            print(f"  - {name}: {details.get('description', 'No description available.')}")
        return
    ...
    if args.target == 'all':
        ...
        for target_name in build_order:
            if target_name in config['targets']:
                ...
                print(f"Warning: Target '{target_name}' specified in 'all' but not found in config.")
        print("\n--- Full Build Finished ---")
    else:
