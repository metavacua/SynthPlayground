...
class TestMasterControlGraphIntegrated(unittest.TestCase):
    ...
    def test_full_integrated_workflow(self):
        ...
        # Check that the post-mortem was created and contains the template text
        self.assertTrue(os.path.exists(self.postmortem_file), f"Post-mortem file was not created at {self.postmortem_file}")
        with open(self.postmortem_file, 'r') as f:
            postmortem_content = f.read()
        self.assertIn("[TASK_ID]", postmortem_content, "The generated post-mortem file does not contain the template placeholder.")
