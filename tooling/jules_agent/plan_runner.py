...
def parse_executable_plan(plan_content: str) -> List[ExecutableCommand]:
    ...
    for block in plan_blocks:
        ...
        if command_part_match:
            ...
            command_lines = [
                line
                for line in command_part.split("\n")
    ...
    try:
        ...
        if result.returncode != 0:
            action_logger.log_action(
                f"Command failed: {command.tool_name} {command.args_text}", cwd
            )
            return False
        ...
        if command.verification_command:
            ...
            if verification_result.returncode != 0:
                action_logger.log_action(
                    f"Verification failed for command: {command.tool_name}", cwd
                )
                return False
        ...
        action_logger.log_action(
            f"An unexpected error occurred during execution: {e}", cwd
        )
        return False
    ...
    for command in commands:
        ...
        if not success:
            if command.on_failure_plan:
                ...
                action_logger.log_action(
                    "Command failed and no failure plan was provided. Halting.", cwd
                )
                break
