...
def update_rule_in_protocol(protocol_id: str, rule_id: str, new_description: str, protocols_dir: str):
    """
    ...
    try:
        ...
        if "rules" not in data or not isinstance(data["rules"], list):
            print(f"Error: Protocol '{protocol_id}' does not contain a valid 'rules' list.")
            exit(1)
        ...
        if not rule_found:
            print(f"Error: Rule with ID '{rule_id}' not found in protocol '{protocol_id}'.")
            exit(1)

        with open(protocol_file, "w") as f:
            json.dump(data, f, indent=2)

        print(f"Successfully updated rule '{rule_id}' in protocol '{protocol_id}' with new description.")
...
def main():
    ...
    # --- 'update-rule' command ---
    parser_update_rule = subparsers.add_parser("update-rule", help="Update a rule's description in a protocol.")
    parser_update_rule.add_argument("--protocol-id", required=True, help="The ID of the protocol to modify.")
    parser_update_rule.add_argument("--rule-id", required=True, help="The ID of the rule to update.")
    parser_update_rule.add_argument("--description", required=True, help="The new description for the rule.")
    ...
    if args.command == "add-tool":
        ...
        update_rule_in_protocol(args.protocol_id, args.rule_id, args.description, args.protocols_dir)
