...
class TestSelfImprovementCLI(unittest.TestCase):
    ...
    def setUp(self):
        ...
        log_entries = [
            # An efficient task with only one plan update
            {
                ...
                "action": {"details": {"tool_name": "set_plan"}},
            },
            {
                "task": {"id": "task-efficient-1"},
                "action": {"details": {"tool_name": "read_file"}},
            },
            ...
            {
                "task": {"id": "task-inefficient-1"},
                "action": {"details": {"tool_name": "list_files"}},
            },
            {
                "task": {"id": "task-inefficient-1"},
                "action": {"details": {"tool_name": "set_plan"}},
            ...
            # An inefficient task with two plan revisions via the
            # 'PLAN_UPDATE' action type
            {"task": {"id": "task-inefficient-2"}, "action": {"type": "PLAN_UPDATE"}},
            {"task": {"id": "task-inefficient-2"}, "action": {"type": "PLAN_UPDATE"}},
            # A task with no planning actions at all
            {
                "task": {"id": "task-no-plan"},
                "action": {"details": {"tool_name": "run_in_bash_session"}},
            },
        ]
    ...
    def test_analyze_planning_efficiency(self):
        """
        Test that the analysis correctly identifies tasks with multiple plan
        updates.
        """
        ...
        self.assertEqual(
            len(inefficient_tasks),
            2,
            "Should only find the two inefficient tasks",
        )
