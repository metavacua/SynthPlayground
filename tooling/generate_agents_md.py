...
def generate_agents_md():
    ...
    if not os.path.isdir(protocol_dir):
        logging.warning(f"CHC protocol directory not found at '{protocol_dir}'. No protocols will be listed.")
    ...
    for root, _, files in os.walk(protocol_dir):
        if "proof.py" in files:
            ...
            try:
                ...
                logging.error(f"Protocol in '{module_path}' is malformed (e.g., missing 'Protocol' class or required methods): {e}")
            except Exception as e:
                logging.error(f"An unexpected error occurred while loading protocol from '{module_path}': {e}")
