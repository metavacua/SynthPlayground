...
def _validate_action(line_num, line_content, state, fsm, fs, placeholders):
    ...
    if action_type not in (transitions or {}):
        message = (
            f"Error on line {line_num+1}: Invalid FSM transition. "
    ...
    if command == "create_file_with_block" and args[0] in fs:
        message = (
            f"Error on line {line_num+1}: Semantic error. "
            f"Cannot create '{args[0]}' because it already exists."
        )
        print(message, file=sys.stderr)
        sys.exit(1)
    if (
        ...
        message = (
            f"Error on line {line_num+1}: Semantic error. "
            f"Cannot access '{args[0]}' because it does not exist."
        )
        print(message, file=sys.stderr)
        sys.exit(1)
    ...
    print(
        f"  Line {line_num+1}: OK. Action '{command}' ({action_type}) "
        f"transitions from {state} -> {next_state}"
    )
...
def validate_plan(plan_filepath):
    ...
    if final_state in fsm["accept_states"]:
        ...
        message = (
            f"\nValidation failed. Plan ends in non-accepted state: " f"'{final_state}'"
        )
        print(message, file=sys.stderr)
        sys.exit(1)
