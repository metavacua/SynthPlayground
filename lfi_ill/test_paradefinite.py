...
class TestParadefiniteInterpreter(unittest.TestCase):
    ...
    def test_co_negation_operator(self):
        ...
        self.assertEqual(self._run_interp(CoNegation(p_true), env).value, ParaconsistentTruth.FALSE)
        self.assertEqual(self._run_interp(CoNegation(p_false), env).value, ParaconsistentTruth.TRUE)
        self.assertEqual(self._run_interp(CoNegation(p_both), env).value, ParaconsistentTruth.BOTH)
        self.assertEqual(self._run_interp(CoNegation(p_neither), env).value, ParaconsistentTruth.NEITHER)
    ...
    def test_undeterminedness_operator(self):
        ...
        self.assertEqual(self._run_interp(Undeterminedness(p_true), env).value, ParaconsistentTruth.TRUE)
        self.assertEqual(self._run_interp(Undeterminedness(p_false), env).value, ParaconsistentTruth.TRUE)
        self.assertEqual(self._run_interp(Undeterminedness(p_both), env).value, ParaconsistentTruth.FALSE)
        self.assertEqual(self._run_interp(Undeterminedness(p_neither), env).value, ParaconsistentTruth.TRUE)
