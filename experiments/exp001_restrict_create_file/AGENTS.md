...
```json
{
  ...
  "rules": [
    ...
    {
      "rule_id": "forbid-create-file",
      "description": "The `create_file_with_block` tool is forbidden. Alternative methods must be used for file creation.",
      "enforcement": "The agent must not use the `create_file_with_block` tool. The orchestrator should block any attempt to call it."
    }
  ],
  "associated_tools": [
    "set_plan",
