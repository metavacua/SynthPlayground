...
def parse_sequent(s: str):
    parts = s.split('|-')
    if len(parts) != 2: raise ValueError("Sequent must be 'Γ |- A'")
    antecedent = tuple(parse_prop(p) for p in parts[0].split(',') if p.strip())
    succedent = (parse_prop(parts[1]),)
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        ...
        print(f"I Logic Interpreter: Attempting to prove: {program_str}")
        try:
